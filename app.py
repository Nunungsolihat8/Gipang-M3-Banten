...
# Injeksi Gaya Desain SaaS Premium Kelas Enterprise
st.markdown("""
<style>
...
# ==========================================
# 2. SEED STATE DATA & KONEKSI DATABASE
# ==========================================
NAMA_SPREADSHEET = "Database_Portofolio_Pengawas"

# Mengamankan Inisialisasi State agar Tidak Terjadi Nilai Kosong
...
# ==========================================
# 3. GLOBAL NAV-BAR & AUTHENTICATION POP-UP
# ==========================================
st.markdown("""
            ...
            if client:
                ...
                if not df_users.empty:
                    ...
                    if not user_match.empty:
                        if user_match.iloc[0].get('Status_Akun', 'Pending') == 'Aktif':
                            st.session_state.logged_in = True
                            st.session_state.user_role = user_match.iloc[0]['Role']
                            st.session_state.asal_sekolah = user_match.iloc[0]['Asal_Sekolah']
                            st.session_state.user_name = f"{user_match.iloc[0]['Nama_Lengkap']}"
                            st.session_state.show_login = False
                            st.rerun()
                        else:
...
if st.session_state.show_register and not st.session_state.logged_in:
    with st.container(border=True):
        ...
        with st.form("form_registrasi"):
            ...
            reg_pass = c4.text_input("🔑 Password (1 Kapital & 1 Angka)", type="password")
                ...
                if reg_nama and reg_sekolah and reg_email and reg_pass:
                    if any(char.isupper() for char in reg_pass) and any(char.isdigit() for char in reg_pass) and len(reg_pass) >= 6:
                        if client:
                            ...
                            st.success("✅ Terdaftar! Silakan tunggu konfirmasi Pengawas.")
                        else:
                            st.error("Database terputus.")
                    else:
                        st.error("❌ Password minimal 6 karakter, wajib 1 Kapital & 1 Angka!")

# ==========================================
# 4. ISOLASI STRUKTUR TOTAL: PUBLIC VS PRIVATE VIEW
# ==========================================
if not st.session_state.logged_in:
    # --------------------------------------
    ...
    st.markdown("<div id='portofolio'></div><br><hr style='opacity:0.5;'><br><h4>📸 Galeri Portofolio Kegiatan Binaan</h4>", unsafe_allow_html=True)
    ...
    # --------------------------------------
    # HALAMAN INTERNAL WORKSPACE (SUDAH LOGIN)
    # --------------------------------------
    ...
    # Penentuan Menu Berdasarkan Role Unik
    if st.session_state.user_role == "Admin":
        sub_menu = st.sidebar.radio("Menu Pengawas", ["Dashboard Admin", "Cek Artefak Sekolah", "Sistem Reminder Email", "Validasi Akun Baru", "Manajemen Tagihan Tugas", "Upload Materi Pusat", "Respon Konsultasi", "Upload Galeri Kegiatan", "🚪 Keluar"])
    elif st.session_state.user_role == "Kepala Sekolah":
        sub_menu = st.sidebar.radio("Menu Kepala Sekolah", ["Dashboard Kepala Sekolah", "Pakta Integritas & Pengesahan", "Layanan Konsultasi", "Upload Galeri Kegiatan", "🚪 Keluar"])
    elif st.session_state.user_role == "Operator":
        sub_menu = st.sidebar.radio("Menu Operator", ["Dashboard Operator", "Upload Artefak", "Riwayat & Perbaikan", "Upload Galeri Kegiatan", "🚪 Keluar"])
    ...
    if sub_menu == "Dashboard Admin":
        ...
        st.metric("Menunggu Pemeriksaan", f"{pending} Dokumen")
                    ...
                    if st.form_submit_button("Simpan Keputusan", type="primary"):
                        ...
                        for idx, row in enumerate(sheet_art.get_all_records()):
                            if str(row.get('Timestamp')) == str(ts_pilih) and str(row.get('Nama_Sekolah')) == str(sekolah_pilih):
                                ...
                                st.success("Keputusan Berhasil Disimpan!")
                                st.rerun()
            else: st.success("Semua dokumen telah selesai diperiksa.")
        ...
        if not df_tagihan.empty and not df_user.empty:
            tugas_pilih = st.selectbox("Pilih Tugas", df_tagihan['Nama_Tugas'].tolist())
            sekolah_sudah = df_art[df_art['Nama_Tugas'] == tugas_pilih]['Nama_Sekolah'].unique().tolist() if not df_art.empty else []
            sekolah_belum = df_user[(df_user['Role'].isin(['Kepala Sekolah', 'Operator'])) & (df_user['Status_Akun'] == 'Active' or df_user['Status_Akun'] == 'Aktif') & (~df_user['Asal_Sekolah'].isin(sekolah_sudah))]
            if not sekolah_belum.empty:
                daftar_email = ",".join(sekolah_belum['Email'].tolist())
                mailto_link = f"mailto:?bcc={daftar_email}&subject=REMINDER&body=Segera+lengkapi+tugas."
                st.markdown(f'<a href="{mailto_link}" target="_blank"><button style="background-color: #d93025; color: white; border: none; padding: 10px 20px; border-radius: 8px;">📧 Kirim Reminder Massal</button></a>', unsafe_allow_html=True)
        ...
        if not df_users.empty:
            ...
            if not df_pending.empty:
                with st.form("form_setujui"):
                    email_pilihan = st.selectbox("Pilih Email", df_pending['Email'].tolist())
        ...
        st.title("📸 Upload Dokumentasi Foto Kegiatan Mandiri")
        with st.container(border=True):
            ...
            if st.button("🚀 Upload ke Portofolio", type="primary"):
                if desc_keg and uploaded_photos:
                    if len(uploaded_photos) <= 3:
                        ...
                        client.open(NAMA_SPREADSHEET).worksheet("Galeri_Portofolio").append_row([datetime.datetime.now().strftime("%Y-%m-%d"), st.session_state.asal_sekolah if st.session_state.asal_sekolah else "Pengawas Pusat", desc_keg, urls[0], urls[1], urls[2]])
                        st.success("🎉 Foto kegiatan Anda berhasil tayang di Galeri Publik!")
                        st.balloons()
                    else: st.error("Maksimal 3 foto.")
        ...
        is_signed = not df_sah.empty and st.session_state.asal_sekolah in df_sah['Asal_Sekolah'].values
        if is_signed:
                    ...
                    if not df_cek.empty and len(df_cek[(df_cek['Nama_Sekolah'] == st.session_state.asal_sekolah) & (df_cek['Nama_Tugas'] == tugas)]) > 0:
                        for idx, row in enumerate(records):
                            if str(row.get('Nama_Sekolah')) == str(st.session_state.asal_sekolah) and str(row.get('Nama_Tugas')) == str(tugas):
                                ...
                                st.success("✅ Berkas revisi berhasil ditimpa ke baris yang sama (Anti-Double)!")
                                break
                    else:
        ...
        if not df_art.empty:
            ...
            if not df_rev.empty:
                for idx, row in df_rev.iterrows():
                    st.error(f"❌ Tugas: {row['Nama_Tugas']} | Catatan: {row['Catatan_Pengawas']}")
